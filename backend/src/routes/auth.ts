...
async function persistRefreshToken(userId: string, refreshToken: string, req: Request) {
  ...
  await pool.query(
    `INSERT INTO user_sessions (user_id, refresh_token_hash, ip, user_agent, expires_at)
     VALUES ($1,$2,$3,$4,$5)`,
    [userId, hash, req.ip || null, req.headers["user-agent"] || null, expiresAt]
  );
}
  ...
  app.post("/api/v1/auth/refresh", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const sessionResult = await pool.query(
        "SELECT id, expires_at FROM user_sessions WHERE user_id = $1 AND refresh_token_hash = $2",
        [payload.sub, hash]
      );
