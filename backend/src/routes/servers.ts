...
export function registerServerRoutes(app: Express) {
  app.get("/api/v1/servers", requireAuth, async (_req: Request, res: Response) => {
    const result = await pool.query(
      "SELECT id,name,description,type,minecraft_version,java_version,ram_mb,port,install_state,power_state FROM servers ORDER BY created_at DESC"
    );
    return res.json({ servers: result.rows });
  });

  app.post("/api/v1/servers", requireAuth, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO servers (owner_user_id,name,description,type,minecraft_version,java_version,ram_mb,port,install_state,power_state,modpack_id,world_profile_id)
      ...
      await client.query(
        "UPDATE servers SET install_state = 'ready', power_state = 'running', updated_at = NOW() WHERE id = $1",
        [server.id]
      );
      await client.query("COMMIT");

      const finalServerResult = await pool.query(
        "SELECT id,name,description,type,minecraft_version,java_version,ram_mb,port,install_state,power_state FROM servers WHERE id = $1",
        [server.id]
      );
  ...
  app.post("/api/v1/servers/:id/power", requireAuth, async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    try {
      await powerServerContainer(id, action as any);
      await pool.query(
        "UPDATE servers SET power_state = $2, updated_at = NOW() WHERE id = $1",
        [id, action === "start" || action === "restart" ? "running" : "offline"]
      );
